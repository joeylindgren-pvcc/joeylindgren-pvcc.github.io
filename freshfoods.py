...
def display_results():
    ...
    titles2 = "Fed Inc Tax" + tab + "State Inc Tax" + tab + "Soc Sec" + tab + "Medicare" + tab + "Ret401k" + tab + "Net"
    print(titles1 + titles2)


    for i in range(num_emps):
        data1 = emp[i] + " " + job[i] + format(gross_pay[i], currency)
        data2 = emp[i] + " " + job[i] + format(fed_tax[i], currency)
        data3 = emp[i] + " " + job[i] + format(state_tax[i], currency)
        data4 = emp[i] + " " + job[i] + format(soc_sec[i], currency)
        data5 = emp[i] + " " + job[i] + format(medicare[i], currency)
        data6 = emp[i] + " " + job[i] + format(ret401k[i], currency)
        data7 = emp[i] + " " + job[i] + format(net_pay[i], currency)
        print(data1)
    print(line)
